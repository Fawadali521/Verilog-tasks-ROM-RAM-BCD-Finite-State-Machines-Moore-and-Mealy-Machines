...
static void eval_int(void *param)
{
    ...
    /* Display the values */
    sprintf(msgbuf, "int_a = %d\n", val_a);
    mti_PrintMessage(msgbuf);
    sprintf(msgbuf, "int_b = %d\n", val_b);
    mti_PrintMessage(msgbuf);
    sprintf(msgbuf, "int_out = %d\n", val_out);
    mti_PrintMessage(msgbuf);
}
...
static void eval_float(void *param)
{
    ...
    val_out = val_a + val_b;
    mti_ScheduleDriver(ip->float_out, (long)(&val_out), 0, MTI_INERTIAL);
