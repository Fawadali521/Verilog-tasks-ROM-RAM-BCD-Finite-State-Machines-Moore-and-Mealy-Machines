...
static mvl9_char[] = { 'U', 'X', '0', '1', 'Z', 'W', 'L', 'H', '-' };
        ...
        if ( cur_testpoint->type == DRIVE ) {
            if ( ! is_array ) {
                ...
                convert_enums_to_mvl9_string( cur_testpoint->test_val,
                                             tmpstring, width );
                if ( ip->verbose ) {
                    sprintf(buf,"TIME %d: drive signal array %s with value %s\n",
            ...
            if ( ! is_array ) {
                ...
                if ( ip->verbose ) {
                    sprintf(buf,"TIME %d: test signal %s for value %c\n",
                           now, tester_ports[portnum].name, exp);
                    mti_PrintMessage(buf);
                }
                if ( sigval != (long) *cur_testpoint->test_val ) {
                    sprintf( buf,
                            "Miscompare at time %d, signal %s. "
                            "Expected \'%c\', Actual \'%c\'\n",
                ...
                if ( ip->verbose ) {
                    sprintf(buf,"TIME %d: test signal %s for value %s\n",
