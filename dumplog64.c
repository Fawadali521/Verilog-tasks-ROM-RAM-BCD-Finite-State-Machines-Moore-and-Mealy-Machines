...
//  Note the CHECK_UNIQUE is not needed as this is called from an iterator
...
/******************************************************************************
//  printSubElements
//  Recursively print the context tree invofmation starting at top and
    ...
    WlfSymbolListId *tail ) 
{
    ...
    iter = wlfSymChildren( top, wlfSelAll );
    if ( iter == NULL ) return;
...
/******************************************************************************
//  sigCb
//  This function prints the time, name of signal and its value.
    ...
    WlfCallbackReason reason) 
{
    ...
    WlfTime64 time;
    WlfDataType dt;
    char *f, fname[1024], value[1024];

    if (reason != WLF_EVENT) 
        return WLF_CONTINUE_SCAN;

    wlfPackTime( pack, &time);
    t =  wlfSymPropTypeId( s, WLF_PROP_TYPE_ID);
    dt = wlfTypePropDataType( t, WLF_TYPE_TYPE);
    if ( (dt == wlfTypeReal) || ( dt == wlfTypeScalar)) {
        radix = WLF_RADIX_DECIMAL;
        size = 12; /* max size of real or integer display */
    } else {
        size = wlfTypePropInt( t, WLF_TYPE_REGISTER_WIDTH);
    }
    
    if ((f=wlfSymPropString(s, WLF_PROP_SYMBOL_FULLNAME))!=NULL) 
        strcpy(fname,f);
    if ((f=wlfValueToString( v, radix, MAX_VALUE_SIZE ))!=NULL)  
        strcpy(value,f);
    ...
    WlfSymbolListId head) 
{
    ...
    for( tlist = head;tlist != NULL; tlist = tlist->next) {
        sym = tlist->symbolId;
        if ( ! sym) break;
        if ( wlfSymIsSymbolSelect( sym, wlfSelAllSignals)) {
...
int main( int argc, char **argv) 
{
    ...
    if ( argc < 2 ) {
        printf("Usage: %s <WLF-file>\n", argv[0]);
        exit(1);
    }

    /* Initialize the WLF api */
    status = wlfInit();
    if ( status != WLF_OK) errorExit("wlfInit");

    /* Open the WLF File */
    wlfFile = wlfFileOpen( argv[1], "vsim_wlf");
    if ( wlfFile == NULL) errorExit("wlfFileOpen");

    /* Check the API version */
    status = wlfFileInfo( wlfFile, &fileInfo);
    if ( status != WLF_OK) errorExit("wlfFileInfo");
    ...
    if ( status != WLF_OK ) errorExit("wlfSymbolListInit");
    ...
    pack = wlfPackCreate();
    if (pack == NULL) errorExit("wlfPackCreate");
    ...
    if ( status != WLF_OK) errorExit("wlfReadDataOverRange");

    /* close the file and release resources */
    status  = wlfFileClose( wlfFile );
    if ( status != WLF_OK) errorExit("wlfFileClose");

    status = wlfCleanup();
    if ( status != WLF_OK) errorExit("wlfCleanup");
