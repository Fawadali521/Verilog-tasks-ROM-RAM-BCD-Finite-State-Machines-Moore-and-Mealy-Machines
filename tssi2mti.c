	...
	char *time;
{
	...
	if (sigp) {
		...
		switch (val) {
			...
			case 'f':
			case 'F':	val_str = (sigp->nbits == 4) ? "1111" : "Zuuu" ; break;
			...
			case '?':
				...
				for (bit = 0; bit < sigp->nbits; bit++) {
					val_buf[bit] = (sigp->bits[bit].dir == 'B') ? 'Z' : 'u';
				}
		}
		for (bit = 0; bit < sigp->nbits; bit++) {
	...
	FILE *forces;
{
	...
	if ((rv == EOF)
	&&  (strcmp(prev_time, last_run_time))) {
		fprintf(forces, "run @%s\n", prev_time);
