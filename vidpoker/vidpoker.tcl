...
proc Create_UI win {
    global bankroll betsize game gameFelt statusLine defbg font
...
### no cheat signal in design */
    ...
    button $pf.b1 -text Deal -command {pushPokerDeal}                 -font $font
    button $pf.b2 -text Draw -command {pushPokerDraw} -state disabled -font $font
    ...
    label $win.l.b.bet -text "Bet" -font $font
    label $win.l.b.l -text "$"     -font $font
    ...
    label $win.l.c.bankroll -text "Bankroll" -font $font
    label $win.l.c.dollar -text "$"          -font $font
    label $win.l.c.cash -textvariable bankroll -relief flat -justify right -font $font
    pack $win.l.c.bankroll -side top
    pack $win.l.c.cash $win.l.c.dollar -side right -expand 1 -fill x

    button $win.l.x.exit -text "Quit" -command {destroy $globalWin} -font $font
    pack $win.l.x.exit -expand 1 -fill both

    frame $win.s -relief sunken -bd 2
    pack $win.s -side bottom -expand 1 -fill both
    text $win.s.t -relief raised -bd 2 -yscrollcommand "$win.s.s set" -height 3 -font $font
...
proc Main {} {
    ...
    wm title $globalWin "ModelSim - Video Poker Example"
